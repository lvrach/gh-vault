/**
 * PR domain API functions - fetch data from GitHub and return structured types.
...
export async function getPr(input: GetPrInput): Promise<PullRequest> {
...
export async function listPrs(input: ListPrsInput): Promise<PullRequestListItem[]> {
  const client = await createGitHubClient();
...
export async function listPrComments(input: ListCommentsInput): Promise<PrComment[]> {
  const client = await createGitHubClient();
...
export async function listPrReviews(input: ListReviewsInput): Promise<PrReview[]> {
  const client = await createGitHubClient();
...
export async function listPrFiles(input: ListFilesInput): Promise<PrFile[]> {
  const client = await createGitHubClient();
...
export async function createPrComment(input: CreateCommentInput): Promise<PrComment> {
  const client = await createGitHubClient();
...
export async function updatePrComment(input: UpdateCommentInput): Promise<PrComment> {
  const client = await createGitHubClient();
...
export async function deletePrComment(input: DeleteCommentInput): Promise<void> {
  const client = await createGitHubClient();
...
export async function createPrReview(input: CreateReviewInput): Promise<PrReview> {
  const client = await createGitHubClient();
...
export async function createPr(input: CreatePrInput): Promise<CreatedPr> {
  const client = await createGitHubClient();
 ...
 */
export async function editPr(input: EditPrInput): Promise<EditPrResult> {
  const client = await createGitHubClient();
 ...
 */
export async function mergePr(input: MergePrInput): Promise<MergeResult> {
  const client = await createGitHubClient();
 ...
 */
export async function enableAutoMerge(input: AutoMergeInput): Promise<AutoMergeResult> {
  const client = await createGitHubClient();
 ...
 */
export async function disableAutoMerge(input: {
  owner: string;
  repo: string;
  pullNumber: number;
}): Promise<void> {
 ...
 */
export async function updatePrState(input: UpdatePrStateInput): Promise<PullRequest> {
  const client = await createGitHubClient();
 ...
 */
export async function updatePrDraft(input: UpdatePrDraftInput): Promise<PullRequest> {
  const client = await createGitHubClient();
  ...
  if (input.draft) {
    ...
    `;
    await client.graphql(mutation, { pullRequestId: pr.node_id });
  } else {
    const mutation = `
  ...
  return getPr({ owner: input.owner, repo: input.repo, pullNumber: input.pullNumber });
}

/**
 * Get check runs and commit statuses for a PR.
 */
export async function listPrChecks(input: ListPrChecksInput): Promise<PrChecksResult> {
  const client = await createGitHubClient();
  ...
  if (input.required) {
    try {
      const { data: protection } = await client.rest.repos.getBranchProtection({
        owner: input.owner,
        repo: input.repo,
        branch: baseBranch,
      });
  ...
  const { data: checksData } = await client.rest.checks.listForRef({
    owner: input.owner,
    repo: input.repo,
    ref: sha,
  ...
  const { data: statusData } = await client.rest.repos.getCombinedStatusForRef({
    owner: input.owner,
    repo: input.repo,
    ref: sha,
  });
...
export async function getPrStatus(input: {
  ...
  const createdByYou = await listPrs({
    owner: input.owner,
    repo: input.repo,
    state: 'open',
  ...
  const { data: reviewData } = await client.rest.search.issuesAndPullRequests({
    q: reviewQuery,
  ...
  const assignedToYou = await listPrs({
    owner: input.owner,
    repo: input.repo,
    state: 'open',
